...
mongoose.connect("mongodb+srv://salmanshan:salman642001@cluster0.odxej1b.mongodb.net/BlogUsersXXDB?retryWrites=true&w=majority&appName=Cluster0")
    ...
    blogmodel.find(
        ...
        (response)=>{
           ...
           if (response.length>0) {
               ...
               // now we get the hased password so we can compare now
               bcrypt.compare(input.password,dbpassword,(error,isMatch)=>{
                ...
                res.json({"status":"user not fount"})
           }
        }
    ).catch()
})
